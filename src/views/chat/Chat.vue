...
export default {
  ...
  methods: {
    send() {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) === 'undefined') {
          ...
          const message = { from: this.$store.getters.regionalId, to: this.chatUser, text: this.text }
          socket.send(JSON.stringify(message)) // 将组装好的json发送给服务端，由服务端进行转发
          this.messages.push({ user: this.$store.getters.regionalId, text: this.text })
          // 构建消息内容，本人消息
          this.createContent(null, this.$store.getters.regionalId, this.text)
          this.text = ''
        }
      }
    },
    ...
    init() {
      ...
      const username = this.$store.getters.regionalId
      const _this = this
      if (typeof (WebSocket) === 'undefined') {
        console.log('您的浏览器不支持WebSocket')
      } else {
        console.log('您的浏览器支持WebSocket')
        const socketUrl = 'ws://localhost:8010/imserver/' + username
        if (socket != null) {
          socket.close()
          socket = null
        }
